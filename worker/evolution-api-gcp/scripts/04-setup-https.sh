...
cat > /tmp/setup-ssl-vm.sh << 'EOFSCRIPT'
#!/bin/bash
set -e
...
cd /opt/evolution-api-gcp 2>/dev/null || true
...
(crontab -l 2>/dev/null | grep -v "certbot renew" || true; echo "0 3 * * * certbot renew --quiet --deploy-hook 'cd /opt/evolution-api-gcp && docker compose restart nginx'") | crontab -

echo "✅ Certificado SSL configurado!"
EOFSCRIPT

# Copiar script para VM
gcloud compute scp /tmp/setup-ssl-vm.sh ${VM_NAME}:~/setup-ssl.sh --zone=${ZONE} --project=${PROJECT_ID}
