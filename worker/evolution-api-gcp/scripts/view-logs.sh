...
if [ -z "$SERVICE" ]; then
  # Ver todos os logs
  gcloud compute ssh ${VM_NAME} --zone=${ZONE} --project=${PROJECT_ID} --command="cd /opt/evolution-api-gcp && sudo docker compose logs -f"
else
  # Ver logs de um serviço específico
  gcloud compute ssh ${VM_NAME} --zone=${ZONE} --project=${PROJECT_ID} --command="cd /opt/evolution-api-gcp && sudo docker compose logs -f ${SERVICE}"
fi
