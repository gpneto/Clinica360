...
# Script completo para fazer deploy do Evolution API v2 no GCP
...
echo "🚀 Deploy completo do Evolution API v2 no GCP"
echo "=============================================="
echo ""
...
# Passo 1: Verificar disco persistente (compartilhado com versão original)
echo ""
echo "📦 Passo 1/6: Verificando disco persistente (compartilhado)..."
...
# Passo 5: Deploy (build local + upload imagem + deploy)
echo ""
echo "🚀 Passo 5/6: Fazendo deploy (build local + upload imagem)..."
echo "   Isso inclui: build local da imagem, upload para servidor, e inicialização dos containers"
...
echo ""
echo "📚 Documentação:"
echo "   - Ver logs: gcloud compute ssh ${VM_NAME} --zone=${ZONE} --command='cd /opt/evolution-api-v2-gcp && sudo docker compose logs -f'"
echo "   - Reiniciar: gcloud compute ssh ${VM_NAME} --zone=${ZONE} --command='cd /opt/evolution-api-v2-gcp && sudo docker compose restart'"
