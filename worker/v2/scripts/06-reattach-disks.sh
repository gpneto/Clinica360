...
# Script para reanexar discos persistentes a uma VM existente (v2)
...
# Criar diretório de montagem (usando o mesmo caminho da versão original)
...
if [ -b /dev/disk/by-id/google-evolution-data-disk ]; then
    if ! mountpoint -q /mnt/disks/evolution-data; then
        ...
        echo 'Montando disco (compartilhado com versão original)...'
...
# Verificar montagem
echo ''
echo '📊 Status do disco montado (compartilhado):'
...
echo "      gcloud compute ssh ${VM_NAME} --zone=${ZONE} --command='cd /opt/evolution-api-v2-gcp && sudo docker compose restart'"
