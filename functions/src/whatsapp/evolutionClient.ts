...
interface EvolutionInstanceStatus {
  instanceName: string;
  status: 'created' | 'connected' | 'connecting' | 'disconnected' | 'qrcode';
  qrcode?: {
    code?: string;
    base64?: string;
    count?: number;
...
export interface EvolutionPairingResult {
  status: 'connected' | 'pending_qr' | 'error';
  qrCode?: string;
  error?: string;
}
...
export async function getOrCreateEvolutionInstance(
  companyId: string,
  integrationType: 'WHATSAPP-BAILEYS' | 'WHATSAPP-BUSINESS' = 'WHATSAPP-BAILEYS',
  number?: string
): Promise<string> {
  ...
  try {
    ...
    const instanceExists = instances?.find((inst: any) => {
          ...
          ...getFetchOptions(apiKey, {
            webhook: {
              ...
              events: [
                ...
                'CHATS_UPSERT',
                'CHATS_DELETE',
                'GROUPS_UPSERT', // Corrigido: era GROUPS_UPDATE
                'GROUP_UPDATE',
                'GROUP_PARTICIPANTS_UPDATE',
                'LABELS_EDIT',
                'LABELS_ASSOCIATION',
                'CALL', // Corrigido: era CALL_UPDATE
                'TYPEBOT_START',
    ...
    if (integrationType === 'WHATSAPP-BUSINESS') {
      ...
      try {
        ...
        if (connectResponse.ok) {
          const connectResult = await connectResponse.json().catch(() => ({})) as any;
      ...
      const webhookResponse = await fetchWithSelfSignedCert(`${apiUrl}/webhook/set/${instanceName}`, {
        ...
        ...getFetchOptions(apiKey, {
          webhook: {
            enabled: true,
            url: webhookUrl,
            webhook_by_events: false, // Enviar todos os eventos
            webhook_base64: false,
            events: [
              'MESSAGES_UPSERT',
              'CONNECTION_UPDATE',
...
export async function getEvolutionQRCode(instanceName: string): Promise<string | null> {
  try {
    ...
    if (response.ok) {
      const testResult = await response.json().catch(() => ({})) as any;
 ...
 */
export async function getEvolutionInstanceStatus(instanceName: string): Promise<EvolutionInstanceStatus | null> {
  try {
    const apiKey = getEvolutionApiKey();
    const apiUrl = getEvolutionApiUrl();
 ...
 */
export async function startEvolutionPairing(
  companyId: string,
  integrationType: 'WHATSAPP-BAILEYS' | 'WHATSAPP-BUSINESS',
  number?: string,
  retryCount: number = 0
    ...
    if (instanceStatus) {
      ...
      if (shouldForceQRCode || connectionState === 'close' || !connectionState || connectionState === 'connecting') {
        ...
        if (!connectResponse.ok) {
          ...
          const testResult = await connectResponse.json().catch(() => ({})) as any;
          ...
          if (testResult && Object.keys(testResult).length === 1 && testResult.count === 0) {
            ...
            const connectResult = testResult as any;
            console.log(`[Evolution] Resposta do connect:`, { 
              keys: Object.keys(connectResult),
              hasQrcode: !!connectResult.qrcode,
 ...
 */
async function updateEvolutionStatus(companyId: string, data: Record<string, unknown>) {
  try {
    await db
