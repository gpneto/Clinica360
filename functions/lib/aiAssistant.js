...
async function checkUserPermission(uid, companyId, requiredActions, authToken) {
    try {
        ...
        if (authToken) {
            ...
            if (hasValidClaims) {
                ...
                if (userCompanyId === companyId) {
                    ...
                    const hasAllPermissions = requiredActions.every((action) => userPermissions.includes(action));
 ...
 */
async function checkFinancialAccess(uid, companyId, authToken) {
    try {
        // PRIORIDADE 1: Tentar usar custom claims do token
        if (authToken) {
            const userRole = authToken.role;
            const userCompanyId = authToken.companyId;
            const userAtivo = authToken.ativo;
...
function getBrazilDates() {
    const now = luxon_1.DateTime.now().setZone(TIMEZONE_BRASIL);
...
function firestoreTimestampToBrazilISO(timestamp) {
    if (!timestamp)
        return undefined;
    ...
    if (timestamp.toDate) {
        ...
        const brazilDate = luxon_1.DateTime.fromJSDate(date).setZone(TIMEZONE_BRASIL);
...
async function saveUsageMetrics(params) {
    try {
        ...
        const todayDate = luxon_1.DateTime.now().setZone(TIMEZONE_BRASIL).startOf('day');
...
async function searchAppointments(params) {
    ...
    try {
                    ...
                    validProfessionalIds.length > 0
                        ? (async () => {
                            try {
                                ...
                                const allDocs = await Promise.all(batches.map(batch => db.collection(`companies/${companyId}/professionals`)
                                    .where(admin.firestore.FieldPath.documentId(), 'in', batch)
                                    .get()));
                                return allDocs.flatMap(snapshot => snapshot.docs.map(doc => ({ id: doc.id, data: doc.data() })));
                            }
                            catch (err) {
                    ...
                    validClientIds.length > 0
                        ? (async () => {
                            try {
                                ...
                                const allDocs = await Promise.all(batches.map(batch => db.collection(`companies/${companyId}/patients`)
                                    .where(admin.firestore.FieldPath.documentId(), 'in', batch)
                                    .get()));
                                return allDocs.flatMap(snapshot => snapshot.docs.map(doc => ({ id: doc.id, data: doc.data() })));
                            }
                    ...
                    validServiceIds.length > 0
                        ? (async () => {
                            try {
                                ...
                                const allDocs = await Promise.all(batches.map(batch => db.collection(`companies/${companyId}/services`)
                                    .where(admin.firestore.FieldPath.documentId(), 'in', batch)
                                    .get()));
                                return allDocs.flatMap(snapshot => snapshot.docs.map(doc => ({ id: doc.id, data: doc.data() })));
                            }
        ...
        let appointments = appointmentDocs.map(doc => {
            const data = doc.data();
        ...
        const normalizeDateForComparison = (dateStr, useEndOfDay = false) => {
            let baseDate;
        ...
        const isSameDay = !!(finalStartDate && finalEndDate && finalStartDate === finalEndDate);
        if (finalStartDate) {
 ...
 */
async function findProfessionalIdByName(companyId, nome) {
 ...
 */
async function getAllActiveProfessionals(companyId) {
    const snapshot = await db
        .collection(`companies/${companyId}/professionals`)
        .where('ativo', '==', true)
        .get();
 ...
 */
async function findPatientIdByName(companyId, nome) {
    try {
 ...
 */
async function findServiceIdByName(companyId, nome) {
    const snapshot = await db
...
async function createAppointment(params) {
    const { companyId, professionalId, professionalName, professionalNumber, clientId, clientName, serviceId, serviceName, inicio, fim, duracaoMinutos, precoCentavos: precoCentavosParam = 0, comissaoPercent = 0, observacoes = '', createdByUid, confirm = false, } = params;
    ...
    if (inicio.includes('Z') || inicio.includes('+') || inicio.includes('-', 10)) {
        ...
        if (match) {
            ...
            const brazilDateTime = luxon_1.DateTime.fromObject({
                year: parseInt(year),
    ...
    let serviceDuracaoMin = duracaoMinutos || serviceData?.duracaoMin || 60;
    let finalPrecoCentavos = precoCentavosParam || serviceData?.precoCentavos || 0;
    ...
    if (fim) {
        ...
        const fimDateTime = luxon_1.DateTime.fromJSDate(fimDate).setZone(TIMEZONE_BRASIL);
    ...
    const hasConflict = conflictingAppointments.docs.some(doc => {
        ...
        return ((inicioDate >= appointmentStart && inicioDate < appointmentEnd) ||
    ...
    const inicioBrazil = luxon_1.DateTime.fromJSDate(inicioDate).setZone(TIMEZONE_BRASIL);
    const fimBrazil = luxon_1.DateTime.fromJSDate(fimDate).setZone(TIMEZONE_BRASIL);
...
async function searchPatients(params) {
    ...
    if (telefone) {
        ...
        query = query.where('telefoneE164', 'in', variants);
    }
...
async function searchProfessionals(params) {
    ...
    if (nome) {
        professionals = professionals.filter((p) => p.apelido?.toLowerCase().includes(nome.toLowerCase()));
    }
...
async function searchServices(params) {
    ...
    if (nome) {
        services = services.filter((s) => s.nome?.toLowerCase().includes(nome.toLowerCase()));
    }
    return services;
}
/**
 ...
 */
exports.aiAssistant = (0, https_1.onCall)({ secrets: [OPENAI_API_KEY], memory: '512MiB' }, async (request) => {
    ...
    const { messages, companyId } = request.data;
    if (!uid) {
    ...
    if (!messages || !Array.isArray(messages) || messages.length === 0) {
        throw new https_1.HttpsError('invalid-argument', 'Mensagens são obrigatórias');
    }
    ...
    try {
        ...
        if (!apiKey) {
            throw new https_1.HttpsError('failed-precondition', 'OpenAI API key não configurada. Por favor, configure o secret OPENAI_API_KEY no Firebase.');
        }
    }
    catch (secretError) {
        console.error('Erro ao obter secret OPENAI_API_KEY:', secretError);
        throw new https_1.HttpsError('failed-precondition', 'Erro ao acessar a API key da OpenAI. Verifique se o secret OPENAI_API_KEY está configurado no Firebase.');
    }
    try {
...
Consulte as descriptions das funções para detalhes específicos. Seja prestativo e objetivo.`,
        ...
        while (assistantMessage.function_call) {
            ...
            try {
                ...
                if (!permissionCheck.allowed) {
                    throw new https_1.HttpsError('permission-denied', `Você não tem permissão para executar esta ação. ${permissionCheck.reason || ''}`);
                }
                ...
                if (functionName === 'getStatistics') {
                    ...
                    if (!hasFinancialAccess) {
                        throw new https_1.HttpsError('permission-denied', 'Você não tem permissão para acessar relatórios e estatísticas. Esta funcionalidade requer acesso financeiro completo.');
                    }
                }
                switch (functionName) {
        ...
        const inputCostUSD = (totalPromptTokens / 1000000) * pricing.input;
        const outputCostUSD = (totalCompletionTokens / 1000000) * pricing.output;
        ...
        if (error?.message?.includes('not a legal HTTP header value')) {
            throw new https_1.HttpsError('failed-precondition', 'API key da OpenAI inválida. Verifique se o secret OPENAI_API_KEY está configurado corretamente no Firebase (sem espaços ou caracteres especiais).');
        }
        if (error?.message?.includes('Connection error') || error?.code === 'ECONNREFUSED') {
            throw new https_1.HttpsError('unavailable', 'Erro de conexão com a API da OpenAI. Por favor, tente novamente em alguns instantes.');
        }
        // Tratar erro de limite de tokens
        if (error?.message?.includes('maximum context length') || error?.message?.includes('tokens')) {
            throw new https_1.HttpsError('resource-exhausted', 'A conversa ficou muito longa. Por favor, inicie uma nova conversa ou faça uma pergunta mais específica.');
        }
        // Tratar outros erros da API OpenAI
        if (error?.status || error?.code) {
            throw new https_1.HttpsError('internal', 'Ocorreu um erro ao processar sua solicitação. Por favor, tente novamente ou reformule sua pergunta.');
        }
        // Erro genérico (não expor detalhes técnicos)
        throw new https_1.HttpsError('internal', 'Ocorreu um erro inesperado. Por favor, tente novamente.');
    }
});
