...
exports.getWhatsAppContactsPhotos = exports.syncWhatsAppContacts = exports.evolutionWebhook = void 0;
const admin = require("firebase-admin");
const https_1 = require("firebase-functions/v2/https");
const https_2 = require("firebase-functions/v2/https");
const firestore_1 = require("firebase-admin/firestore");
const whatsappEnvio_1 = require("./whatsappEnvio");
const whatsappAgendamento_1 = require("./whatsappAgendamento");
...
async function fetchWithSelfSignedCert(url, options = {}, timeoutMs = 30000) {
 ...
 */
async function updateWhatsappPhoneNumberMapping(phoneNumber, companyId) {
    try {
        const normalized = normalizarTelefone(phoneNumber);
        if (!normalized || normalized.length < 10) {
            return;
 ...
 */
async function getParticipantFromEvolutionAPI(instanceName, messageId, remoteJid) {
    try {
 ...
 */
async function findPhoneByPatientName(companyId, patientName) {
    try {
        if (!patientName || !companyId)
            return null;
 ...
 */
async function downloadAndUploadMedia(instanceName, messageId, remoteJid, mediaType, companyId, chatId, mediaUrl, mediaMimetype, mediaSize) {
    try {
        const apiUrl = getEvolutionApiUrl();
        const apiKey = getEvolutionApiKey();
        ...
        const response = await fetchWithSelfSignedCert(downloadUrl, {
            method: 'POST',
            ...getFetchOptions(apiKey, {
                message: {
                    key: {
        ...
        if (mediaType === 'image') {
            ...
            const isValidImage = firstBytes[0] === 0xFF && firstBytes[1] === 0xD8 && firstBytes[2] === 0xFF || // JPEG
 ...
 */
async function fetchAllContacts(instanceName) {
...
async function handleMessage(companyId, message, instanceName) {
    ...
    try {
        ...
        let existingMessageSource = undefined;
        ...
        if (message.message) {
            if (message.message.conversation) {
                ...
                if (instanceName && companyId) {
                    ...
                    try {
                        mediaInfo = await downloadAndUploadMedia(instanceName, wamId, remoteJid, 'image', companyId, chatId, message.message.imageMessage.url, message.message.imageMessage.mimetype, message.message.imageMessage.fileLength);
                ...
                if (instanceName && companyId) {
                    ...
                    try {
                        mediaInfo = await downloadAndUploadMedia(instanceName, wamId, remoteJid, 'video', companyId, chatId, message.message.videoMessage.url, message.message.videoMessage.mimetype, message.message.videoMessage.fileLength);
                ...
                if (instanceName && companyId) {
                    ...
                    try {
                        mediaInfo = await downloadAndUploadMedia(instanceName, wamId, remoteJid, 'audio', companyId, chatId, message.message.audioMessage.url, message.message.audioMessage.mimetype, message.message.audioMessage.fileLength);
                ...
                if (instanceName && companyId) {
                    ...
                    try {
                        mediaInfo = await downloadAndUploadMedia(instanceName, wamId, remoteJid, 'document', companyId, chatId, message.message.documentMessage.url, message.message.documentMessage.mimetype, message.message.documentMessage.fileLength);
        ...
        if (!message.key.fromMe && messageText && messageText.trim() && companyId) {
            try {
                ...
                const agendamentoEnabled = await (0, whatsappAgendamento_1.isAgendamentoWhatsappEnabled)(companyId);
                ...
                if (agendamentoEnabled) {
                    ...
                    const processed = await (0, whatsappAgendamento_1.processAgendamentoMessage)(companyId, chatId, messageText);
                    ...
                    if (!processed) {
                        ...
                        const initialResult = await (0, whatsappAgendamento_1.handleInitialMessage)(companyId, chatId, messageText);
                ...
                console.error(`[Evolution Webhook] Stack trace:`, error.stack);
 ...
 */
async function handleContactsUpdate(companyId, contacts) {
 ...
 */
async function handleChatsUpsert(companyId, chats) {
    ...
    try {
        ...
        for (const chat of chats) {
            ...
            if (chat.unreadMessages > 0) {
                chatData.last_message_at = firestore_1.FieldValue.serverTimestamp();
            }
...
async function processEvolutionWebhook(payload) {
    try {
        ...
        if (payload.event === 'messages.upsert') {
            ...
            let messages = [];
            if (payload.data && typeof payload.data === 'object' && 'messages' in payload.data && Array.isArray(payload.data.messages)) {
        ...
        if (payload.event === 'connection.update' && payload.data && typeof payload.data === 'object' && 'connection' in payload.data) {
            const connectionState = payload.data.connection?.state;
            const lastDisconnectReason = payload.data.connection?.lastDisconnect?.error?.message ||
                payload.data.connection?.lastDisconnect?.error?.toString() ||
                payload.data.connection?.lastDisconnect?.reason ||
                null;
            ...
            if (companyId) {
                if (connectionState === 'open') {
                    ...
                    const updateData = {
                        status: 'disconnected',
                        lastDisconnectAt: firestore_1.FieldValue.serverTimestamp(),
                        qrCode: firestore_1.FieldValue.delete(), // Limpar QR code quando desconectar
                    };
        ...
        if (payload.event === 'qrcode.updated' && payload.data && typeof payload.data === 'object' && 'qrcode' in payload.data) {
            const qrCode = payload.data.qrcode?.code || payload.data.qrcode?.base64;
            console.log(`[Evolution Webhook] 📱 Webhook qrcode.updated (${companyId || 'unknown'})`);
            if (qrCode && companyId) {
                updateStatus(companyId, {
                    status: 'pending_qr',
                    qrCode: qrCode,
                    qrCodeGeneratedAt: firestore_1.FieldValue.serverTimestamp(), // Usar nome consistente com o frontend
                }).catch(console.error);
...
exports.evolutionWebhook = (0, https_1.onRequest)({
    ...
    try {
        ...
        let companyId = companyIdFromPath;
        let processed = false;
        if (companyId) {
 ...
 */
async function fetchMessagesFromAPI(instanceName, remoteJid, page = 1, offset = 10) {
    try {
        const apiKey = getEvolutionApiKey();
        const apiUrl = getEvolutionApiUrl();
 ...
 */
exports.syncWhatsAppContacts = (0, https_2.onCall)({
    region: 'us-central1',
    memory: '1GiB',
    timeoutSeconds: 540,
    maxInstances: 10,
                ...
                if (contactInfo?.profilePicUrl) {
                    ...
                    if (shouldUpdatePhoto) {
                        await contactRef.set({
                            ...
                            remoteJid: contactRemoteJid,
                            profilePicUrlUpdatedAt: firestore_1.FieldValue.serverTimestamp(),
                            updatedAt: firestore_1.FieldValue.serverTimestamp(),
                        }, { merge: true });
                    ...
                    if (foundContact?.profilePicUrl) {
                        ...
                        await contactRef.set({
                            ...
                            remoteJid: foundRemoteJid || contactRemoteJid,
                            profilePicUrlUpdatedAt: firestore_1.FieldValue.serverTimestamp(),
                            updatedAt: firestore_1.FieldValue.serverTimestamp(),
                        }, { merge: true });
 ...
 */
exports.getWhatsAppContactsPhotos = (0, https_2.onCall)({
    region: 'us-central1',
    memory: '512MiB',
    timeoutSeconds: 30,
    maxInstances: 20,
...
//# sourceMappingURL=evolutionWebhook.js.map