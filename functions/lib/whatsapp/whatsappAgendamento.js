...
exports.processAgendamentoMessage = exports.handleInitialMessage = exports.canPatientAgendar = exports.isAgendamentoWhatsappEnabled = void 0;
 ...
 */
function formatNumberOption(num) {
    return `${num}.`;
}
/**
 * Obtém ou cria o contexto de agendamento para um chat
 */
async function getAgendamentoContext(companyId, chatId) {
 ...
 */
async function updateAgendamentoContext(companyId, chatId, updates) {
    try {
        const contextRef = db.collection(`companies/${companyId}/whatsappAgendamentoContext`).doc(chatId);
 ...
 */
async function resetAgendamentoContext(companyId, chatId) {
    try {
        const contextRef = db.collection(`companies/${companyId}/whatsappAgendamentoContext`).doc(chatId);
        const updateData = {
            state: 'initial',
 ...
 */
async function isAgendamentoWhatsappEnabled(companyId) {
    const checkStart = Date.now();
    console.log(`[WhatsApp Agendamento] [${companyId}] isAgendamentoWhatsappEnabled INÍCIO`);
    try {
        const settings = await (0, whatsappEnvio_1.getCompanySettings)(companyId);
...
exports.isAgendamentoWhatsappEnabled = isAgendamentoWhatsappEnabled;
/**
 * Busca paciente pelo número de telefone
 */
async function findPatientByPhone(companyId, phone) {
    try {
        const normalizedPhone = (0, whatsappEnvio_1.normalizePhoneForContact)(phone);
 ...
 */
async function canPatientAgendar(companyId, phone) {
    try {
        const settings = await (0, whatsappEnvio_1.getCompanySettings)(companyId);
...
exports.canPatientAgendar = canPatientAgendar;
/**
 * Envia mensagem via Evolution API
 */
async function sendMessage(companyId, chatId, message) {
    ...
    try {
        await (0, evolutionClient_1.sendEvolutionTextMessage)({
            companyId,
            to: chatId,
            message,
        });
 ...
 */
function addSairOption(message) {
 ...
 */
async function handleSairCommand(companyId, chatId) {
    await sendMessage(companyId, chatId, 'Operação cancelada. Se precisar de algo, é só me chamar! 👋');
 ...
 */
async function handleInitialMessage(companyId, chatId, messageText) {
    const startTime = Date.now();
    console.log(`[WhatsApp Agendamento] [${chatId}] handleInitialMessage INÍCIO - ${new Date().toISOString()}`);
    try {
        // ENVIAR FEEDBACK PRIMEIRO (antes de qualquer validação)
        const feedbackStart = Date.now();
        console.log(`[WhatsApp Agendamento] [${chatId}] Enviando feedback...`);
        await sendMessage(companyId, chatId, '⏳ Processando sua mensagem...');
        console.log(`[WhatsApp Agendamento] [${chatId}] Feedback enviado em ${Date.now() - feedbackStart}ms`);
        // Buscar contexto
        const contextStart = Date.now();
        ...
        if (!canAgendar) {
            await sendMessage(companyId, chatId, 'Olá! O agendamento pelo WhatsApp está disponível apenas para clientes cadastrados. Por favor, entre em contato conosco para se cadastrar.');
            return true;
        }
        // Verificar se passou um dia desde a última atualização
        const agora = luxon_1.DateTime.now().setZone('America/Sao_Paulo');
        ...
        const ultimaAtualizacao = updatedAtDate
            ? luxon_1.DateTime.fromJSDate(updatedAtDate).setZone('America/Sao_Paulo')
            : createdAtDate
                ? luxon_1.DateTime.fromJSDate(createdAtDate).setZone('America/Sao_Paulo')
                : null;
        ...
        if (passouUmDia) {
            ...
            await resetAgendamentoContext(companyId, chatId);
            await sendMessage(companyId, chatId, addSairOption(`Olá! 👋\n\nComo posso ajudá-lo hoje?\n\n1️⃣ *Agendar consulta*\n2️⃣ *Consultar meu agendamento*\n3️⃣ *Falar com atendente*\n\nDigite o número da opção desejada.`));
 ...
 */
async function processAgendamentoMessage(companyId, chatId, messageText) {
    try {
        const context = await getAgendamentoContext(companyId, chatId);
        if (!context) {
            return false;
...
exports.processAgendamentoMessage = processAgendamentoMessage;
/**
 * Processa escolha inicial: agendar, consultar ou falar com atendente
 */
async function handleAgendarOuConsultar(companyId, chatId, text, context) {
    ...
    if (text === '1' || text.includes('agendar') || text.includes('1️⃣')) {
        ...
        if (professionalsSnapshot.empty) {
            await sendMessage(companyId, chatId, addSairOption('Desculpe, não há profissionais disponíveis no momento. Por favor, entre em contato conosco.'));
            await resetAgendamentoContext(companyId, chatId);
            return true;
        }
    ...
    if (text === '3' || text.includes('atendente') || text.includes('falar') || text.includes('3️⃣')) {
        await sendMessage(companyId, chatId, addSairOption('Você será atendido por um de nossos atendentes em breve. Aguarde um momento, por favor.\n\n💡 *Dica:* Digite "voltar", "menu" ou "agendar" a qualquer momento para retornar ao menu automático.'));
        await updateAgendamentoContext(companyId, chatId, { state: 'manual_mode' });
        return true;
    }
    // Se não reconheceu, pedir novamente
    await sendMessage(companyId, chatId, addSairOption('Por favor, escolha uma das opções:\n\n1️⃣ Agendar consulta\n2️⃣ Consultar meu agendamento\n3️⃣ Falar com atendente\n\nDigite o número da opção.'));
    return true;
}
/**
 * Processa seleção de profissional
 */
async function handleSelecionarProfissional(companyId, chatId, text, context) {
    // Verificar se quer sair
    if (isSairCommand(text)) {
        return await handleSairCommand(companyId, chatId);
    }
    ...
    const professionalsSnapshot = await professionalsRef
        ...
        .get();
    if (professionalsSnapshot.empty) {
        await sendMessage(companyId, chatId, 'Desculpe, não há profissionais disponíveis. Voltando ao menu inicial...');
        await resetAgendamentoContext(companyId, chatId);
        return true;
    }
    const professionals = professionalsSnapshot.docs.map((doc, index) => ({
    ...
    if (!isNaN(selectedIndex) && selectedIndex > 0 && selectedIndex <= professionals.length) {
        ...
        const found = professionals.find(p => p.nome.toLowerCase().includes(text) || text.includes(p.nome.toLowerCase()));
        if (found) {
    ...
    if (servicosIds.length > 0) {
        ...
        if (filteredServices.length === 0) {
            await sendMessage(companyId, chatId, addSairOption('Desculpe, não há serviços disponíveis para agendamento no momento.'));
            await resetAgendamentoContext(companyId, chatId);
            return true;
        }
 ...
 */
async function handleSelecionarServico(companyId, chatId, text, context) {
    // Verificar se quer sair
    if (isSairCommand(text)) {
        return await handleSairCommand(companyId, chatId);
    }
    ...
    if (!isNaN(selectedIndex) && selectedIndex > 0 && selectedIndex <= services.length) {
        ...
        const found = services.find(s => s.nome.toLowerCase().includes(text) || text.includes(s.nome.toLowerCase()));
        if (found) {
    ...
    const hoje = luxon_1.DateTime.now().setZone('America/Sao_Paulo');
 ...
 */
async function handleSelecionarData(companyId, chatId, text, context) {
    // Verificar se quer sair
    if (isSairCommand(text)) {
        return await handleSairCommand(companyId, chatId);
    }
    ...
    if (text === 'hoje' || text.includes('hoje')) {
        ...
        if (dateMatch) {
            ...
            selectedDate = luxon_1.DateTime.fromObject({
                year: parseInt(year),
    ...
    if (!selectedDate || !selectedDate.isValid) {
        await sendMessage(companyId, chatId, addSairOption('Data inválida. Por favor, digite a data no formato DD/MM/AAAA (ex: 25/12/2024), "hoje" ou "amanhã".'));
        return true;
    }
    ...
    const horariosDisponiveis = await getHorariosDisponiveis(companyId, context.professionalId, selectedDate.toFormat('yyyy-MM-dd'), context.serviceIds);
    if (horariosDisponiveis.length === 0) {
        await sendMessage(companyId, chatId, addSairOption(`Não há horários disponíveis para ${selectedDate.toFormat('dd/MM/yyyy')}. Por favor, escolha outra data.`));
        return true;
    }
 ...
 */
async function handleSelecionarHorario(companyId, chatId, text, context) {
    // Verificar se quer sair
    if (isSairCommand(text)) {
        return await handleSairCommand(companyId, chatId);
    }
    ...
    const horariosDisponiveis = await getHorariosDisponiveis(companyId, context.professionalId, context.selectedDate, context.serviceIds);
    if (horariosDisponiveis.length === 0) {
    ...
    // Verificar conflitos
    const hasConflict = await verificarConflitoAgendamento(companyId, context.professionalId, context.selectedDate, selectedHorario.inicio, selectedHorario.fim);
    if (hasConflict) {
        await sendMessage(companyId, chatId, addSairOption('Este horário não está mais disponível. Por favor, escolha outro horário.'));
    ...
    const dateObj = luxon_1.DateTime.fromFormat(context.selectedDate, 'yyyy-MM-dd', { zone: 'America/Sao_Paulo' });
 ...
 */
async function handleConfirmarAgendamento(companyId, chatId, text, context) {
    // Verificar se quer sair
    if (isSairCommand(text)) {
        return await handleSairCommand(companyId, chatId);
    }
    ...
    if (!textUpper.includes('CONFIRMAR') && !textUpper.includes('CONFIRM')) {
        await sendMessage(companyId, chatId, addSairOption('Por favor, digite *CONFIRMAR* para confirmar o agendamento ou *CANCELAR* para cancelar.'));
        return true;
    }
    ...
    if (!patientId) {
        ...
        if (!patientId && settings.agendamentoWhatsappApenasContatos) {
            await sendMessage(companyId, chatId, 'Você precisa estar cadastrado para agendar. Por favor, entre em contato conosco para se cadastrar.');
            await resetAgendamentoContext(companyId, chatId);
            return true;
        }
        // Se não encontrou mas pode criar, solicitar nome primeiro
        if (!patientId) {
            await sendMessage(companyId, chatId, addSairOption('Para finalizar o agendamento, precisamos do seu nome para criar seu cadastro.\n\nPor favor, digite seu nome completo:'));
            await updateAgendamentoContext(companyId, chatId, {
    ...
    const hasConflict = await verificarConflitoAgendamento(companyId, context.professionalId, context.selectedDate, context.selectedTime, fim.toFormat('HH:mm'));
    if (hasConflict) {
        await sendMessage(companyId, chatId, 'Este horário não está mais disponível. Por favor, inicie um novo agendamento.');
        await resetAgendamentoContext(companyId, chatId);
        return true;
    }
    // Criar agendamento
    try {
        ...
        const newAppointment = {
            ...
            clientId: patientId,
            serviceId: context.serviceIds[0],
        ...
        const [companyDoc, professionalDoc, clientDoc, settingsDoc, ...serviceDocs] = await Promise.all([
            ...
            ...context.serviceIds.map((serviceId) => db.collection(`companies/${companyId}/services`).doc(serviceId).get())
        ]);
        ...
        await sendMessage(companyId, chatId, 'Desculpe, ocorreu um erro ao criar o agendamento. Por favor, tente novamente ou entre em contato conosco.');
        await resetAgendamentoContext(companyId, chatId);
        return true;
    }
}
 ...
 */
async function handleConsultarAgendamento(companyId, chatId, text, context) {
    // Verificar se quer sair
    if (isSairCommand(text)) {
        return await handleSairCommand(companyId, chatId);
    }
    ...
    if (!patientId) {
        ...
        await sendMessage(companyId, chatId, addSairOption('Não encontramos seu cadastro em nosso sistema.\n\nPara consultar seus agendamentos, precisamos do seu nome para criar seu cadastro.\n\nPor favor, digite seu nome completo:'));
    ...
    if (appointmentsSnapshot.empty) {
        await sendMessage(companyId, chatId, 'Você não possui agendamentos futuros no momento.');
        await resetAgendamentoContext(companyId, chatId);
        return true;
    }
    ...
    for (const appointmentDoc of appointmentsSnapshot.docs) {
        ...
        const dateFormatted = luxon_1.DateTime.fromJSDate(inicio).setZone('America/Sao_Paulo').toFormat('dd/MM/yyyy');
        const timeFormatted = luxon_1.DateTime.fromJSDate(inicio).setZone('America/Sao_Paulo').toFormat('HH:mm');
 ...
 */
async function handleSolicitarNomePaciente(companyId, chatId, text, context) {
    // Verificar se quer sair
    if (isSairCommand(text)) {
        return await handleSairCommand(companyId, chatId);
    }
    ...
    if (!nome || nome.length < 2) {
        await sendMessage(companyId, chatId, addSairOption('Por favor, digite um nome válido (mínimo 2 caracteres).'));
        return true;
    }
    ...
    try {
        ...
        const newPatientRef = await patientsRef.add({
            nome: nome,
            telefoneE164: (0, whatsappEnvio_1.normalizePhoneForContact)(chatId),
        ...
        await sendMessage(companyId, chatId, addSairOption(`✅ Cadastro criado com sucesso, ${nome}!\n\nAgora você pode consultar seus agendamentos ou criar um novo agendamento.`));
        ...
        const appointmentsSnapshot = await appointmentsRef
            ...
            .get();
        if (appointmentsSnapshot.empty) {
            await sendMessage(companyId, chatId, addSairOption('Você não possui agendamentos futuros no momento.\n\nDeseja criar um novo agendamento? Digite 1 para agendar ou 3 para falar com atendente.'));
            await updateAgendamentoContext(companyId, chatId, {
        ...
        await sendMessage(companyId, chatId, 'Desculpe, ocorreu um erro ao criar seu cadastro. Por favor, tente novamente ou entre em contato conosco.');
        await resetAgendamentoContext(companyId, chatId);
        return true;
    }
}
 ...
 */
async function getHorariosDisponiveis(companyId, professionalId, date, // YYYY-MM-DD
serviceIds // IDs dos serviços para calcular duração total
) {
    try {
        // Buscar configurações de horário
        const settingsDoc = await db.collection(`companies/${companyId}/settings`).doc('general').get();
        const settings = settingsDoc.data();
        const horarioConfig = settings?.horarioFuncionamento || {};
        ...
        const dateObj = luxon_1.DateTime.fromFormat(date, 'yyyy-MM-dd', { zone: 'America/Sao_Paulo' });
        ...
        while (currentTime < endTime) {
            ...
            const isOccupied = agendamentosOcupados.some(agendamento => {
                const agendamentoInicio = luxon_1.DateTime.fromJSDate(agendamento.inicio).setZone('America/Sao_Paulo');
                const agendamentoFim = luxon_1.DateTime.fromJSDate(agendamento.fim).setZone('America/Sao_Paulo');
 ...
 */
async function verificarConflitoAgendamento(companyId, professionalId, date, // YYYY-MM-DD
inicio, // HH:mm
fim // HH:mm
) {
    try {
        ...
        for (const doc of appointmentsSnapshot.docs) {
            ...
            if ((inicioDate >= appointmentInicio && inicioDate < appointmentFim) ||
...
//# sourceMappingURL=whatsappAgendamento.js.map